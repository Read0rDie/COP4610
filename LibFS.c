...
static char set_bits(char entry, int nbits);
static void set_bitmap(char map[], int nbits);
static int search_bitmap(char map[], int nbits);
static int search_bits(char entry, int nbits);
static char set_single_bit(char entry, int index);
static int bitmap_reset(int start, int num, int nbits);
...
static void bitmap_init(int start, int num, int nbits)
{
	...
	int sectorNum = start;
	int sectors = num;
	char sectorBitmap[SECTOR_SIZE];
	int nsec = num;
	while(nsec > 0)
	{
		memset(sectorBitmap, 0, SECTOR_SIZE);
		set_bitmap(sectorBitmap, bits);
		Disk_Write(sectorNum, sectorBitmap);
		sectorNum++;
		bits -= (SECTOR_SIZE * 8);
		nsec -= 1;
	}
...
static void set_bitmap(char map[], int nbits)
{
	...
	for(i = 0; i < SECTOR_SIZE; i++)
	{
		...
		{
			if(bits > 8)
				map[i] = set_bits(map[i], 8);
			else
				map[i] = set_bits(map[i], bits);
			bits -= 8;
		}
		else
			break;
...
static int bitmap_first_unused(int start, int num, int nbits)
{
	...
	while(num > 0)
	{
		Disk_Read(sectorNum, sectorBitmap);
		unusedLoc = search_bitmap(sectorBitmap, bits);
		if(unusedLoc > 0)
		{
			if(bitmap_reset(start, num, unusedLoc) == 0)
				return (SECTOR_SIZE * 8 * (sectorNum - start) + unusedLoc);
		}
		sectorNum++;
		bits -= SECTOR_SIZE;
		num -= 1;
...
{
	int i;
	int delta = ibit/(SECTOR_SIZE * 8);
	int sectorNum = start + delta;
	int bit_Location = ibit;
	char sectorBitmap[SECTOR_SIZE];
	if(ibit > 0 && ibit < 10000)
	{
		Disk_Read(sectorNum, sectorBitmap);
		char temp = set_single_bit(sectorBitmap[bit_Location/8], bit_Location % 8);
		sectorBitmap[bit_Location/8] = temp | sectorBitmap[bit_Location/8];
		if(Disk_Write(sectorNum, sectorBitmap) == -1)
		{
			dprintf("Disk Write Failed\n");
			return -1;			
		}
		return 0;
	}	
...
{
	regex_t regexComp;
	char* regex = "[-a-zA-Z_0-9_.]+";
	regcomp(&regexComp, regex, REG_EXTENDED);
	if(strlen(name) < MAX_NAME)
	{
		if((regexec(&regexComp, name, 0, 0, 0)) == 0)
			return 0;
		else
			return 1;	
	}
...
int remove_inode(int type, int parent_inode, int child_inode)
{
	...
	if(bitmap_reset(INODE_BITMAP_START_SECTOR, INODE_BITMAP_SECTORS, child_inode) > 0) return -1;
	...
	{
		if(parent->data[i] == child_inode)
		{
			parent->data[i] = 0;
			parent->size--;
		}
	}
	dprintf("...child inode removed from parent inode\n");
	return 0;  
}
...
int FS_Boot(char* backstore_fname)
{
	...
	else 
	{
		...
		{
      			printf("... check size of file '%s' failed\n", bs_filename);
      			osErrno = E_GENERAL;
      			return -1;
    		}
...
int File_Read(int fd, void* buffer, int size)
{
	...
	int i;
	int j = 0;
	while (remainder > 0 && open_files[fd].pos < MAX_SECTORS_PER_FILE && j < child->size)
	{
		i = open_files[fd].pos;
		data_sector = child->data[j];
		if(Disk_Read(data_sector, data_buffer) < 0) return -1;
		if(remainder >= SECTOR_SIZE)
		{
			if((memcpy(buffer + (i * SECTOR_SIZE),  data_sector, SECTOR_SIZE)) == NULL)
			{
    				dprintf("... fd=%d does not fit in buffer\n", fd);
		...
		{
			if((memcpy(buffer + (i * SECTOR_SIZE),  data_sector, remainder)) == NULL)
			{
    				dprintf("... fd=%d does not fit in buffer\n", fd);
		    		osErrno = E_BUFFER_TOO_SMALL;
    				return -1;
...
int File_Write(int fd, void* buffer, int size)
{
	...
	while (remainder > 0 && open_files[fd].pos < MAX_SECTORS_PER_FILE && child->size < MAX_SECTORS_PER_FILE)
	{
		i = open_files[fd].pos;		
		...
		if(remainder >= SECTOR_SIZE)
		{
			if((memcpy(data_buffer, buffer + (i * SECTOR_SIZE), SECTOR_SIZE)) == NULL)
			{
    				dprintf("... fd=%d does not have enough space\n", fd);
		    		osErrno = E_BUFFER_TOO_SMALL;
    				return -1;
			}
		...
		{
			if((memcpy(data_buffer, buffer + (i * SECTOR_SIZE), remainder)) == NULL)
			{
    				dprintf("... fd=%d does not have enough space\n", fd);
		    		osErrno = E_BUFFER_TOO_SMALL;
    				return -1;
		...
		if(Disk_Write(newsec, data_buffer) == -1)
			return -1;

		remainder -= SECTOR_SIZE;
		open_files[fd].pos++;
    		child->data[child->size] = newsec;
		child->size++;
	}
	return size;
}
...
int Dir_Unlink(char* path)
{
	...
	int child_inode;
	int parent_inode;
	parent_inode = follow_path(path, &child_inode, NULL);
...
int Dir_Read(char* path, void* buffer, int size)
{
  	...
  	if(parent->type != 1) 
	{
  	...
  	int group = parent->size/DIRENTS_PER_SECTOR + 1;
	int i = 0;
	while(group > 0)
	{
	  	char dirent_buffer[SECTOR_SIZE];
  		if(Disk_Read(parent->data[i], dirent_buffer) < 0) return -1;
    		if((memcpy(buffer + (i * SECTOR_SIZE), dirent_buffer, SECTOR_SIZE)) == NULL) return -1;
		i++;
		group--;
	}
	return 0;
