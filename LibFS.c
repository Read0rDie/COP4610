...
static void set_bits(char entry, int nbits);
static void set_bitmap(char map[], int nbits);
static int search_bitamp(char map[], int nbits);
static int search_bits(char entry, int nbits);
static void set_single_bit(char entry, int index);
...
static int bitmap_reset(int start, int num, int ibit)
{
	...
	int bit_Location = ibit % (SECTOR_SIZE * 8);
	char sectorBitmap[SECTOR_SIZE];
	if(ibit > 0 && ibit < num)
	{
		Disk_Read(sectorNum, sectorBitmap);
		set_single_bit(sectorBitmap[bit_Location/8], bit_Location % 8);
		return 0;
	}	
	return -1;
}
...
{
	regex_t regexComp;
	char* regex = "[a-zA-Z_][a-zA-Z_0-9]*\\.[a-zA-Z0-9]+";
...
int File_Read(int fd, void* buffer, int size)
{
	...
	int data_sector;
	int i;
	while (remainder > 0 && open_files[fd].pos < MAX_SECTORS_PER_FILE)
	{
		i = open_files[fd].pos;
		data_sector = INODE_TABLE_START_SECTOR+child->data[i]/INODES_PER_SECTOR;
...
int File_Write(int fd, void* buffer, int size)
{
	...
	while (remainder > 0 && open_files[fd].pos < MAX_SECTORS_PER_FILE)
	{
		...
		if(remainder >= SECTOR_SIZE)
		{
			if((memcpy(data_sector, buffer + (i * SECTOR_SIZE), SECTOR_SIZE)) == NULL)
			{
		...
		{
			if((memcpy(data_sector, buffer + (i * SECTOR_SIZE), remainder)) == NULL)
			{
    				dprintf("... fd=%d does not have enough space\n", fd);
		    		osErrno = E_BUFFER_TOO_SMALL;
    				return -1;
